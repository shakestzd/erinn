...
func (w *Writer) writeBatchAttempt(
	...
	for i := range signals {
		...
		if resolvedEarly {
			agentID = resolveSignalAgentID(ctx, w.conn, s, agentIDByOwnSpan)
			featureID = resolveFeatureID(ctx, w.conn, s.SessionID, agentID, featureCache)
		}
		...
		if s.Kind == otel.KindSpan && s.CanonicalName == otel.CanonicalSubagent && s.SpanID != "" {
			upgraded, upgradeErr := tryUpgradePlaceholder(ctx, w.upgradeStmt, s, attrsJSON, successVal, featureID, agentID)
			if upgradeErr != nil {
