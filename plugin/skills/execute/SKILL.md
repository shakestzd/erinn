...
```
Bash("htmlgraph track show <trk-id>")
```

Parse the text output to extract the track title (appears on the first line after the track ID). Use it (not the raw track ID) as the outer `Agent()` description when spawning the top-level coordinator:
