...
// at .agents/plugins/wipnote/.codex-plugin/plugin.json (inside the marketplace
...
func TestCodexPluginTreeUnderPluginsSubdir(t *testing.T) {
	...
	pluginManifest := filepath.Join(outDir, ".agents", "plugins", "wipnote", ".codex-plugin", "plugin.json")
	...
	oldPluginsPath := filepath.Join(outDir, "plugins", "wipnote", ".codex-plugin", "plugin.json")
	...
	if plug.Skills != "./skills/" {
		t.Errorf("plugin.json skills = %q, want ./skills/", plug.Skills)
	}
	if plug.Hooks != "./hooks.json" {
		t.Errorf("plugin.json hooks = %q, want ./hooks.json", plug.Hooks)
	}
	if plug.MCPServers != "./.mcp.json" {
		t.Errorf("plugin.json mcpServers = %q, want ./.mcp.json", plug.MCPServers)
	}
}
...
// .agents/plugins/wipnote/; leftovers under wipnote/ must be removed, but
...
func TestCodexAdapterRemovesStaleFilesMarketplace(t *testing.T) {
	...
	// Seed a stale file deep inside the .agents/plugins/wipnote/ owned subtree.
	staleFile := filepath.Join(outDir, ".agents", "plugins", "wipnote", "commands", "stale-cmd.md")
	...
	staleDir := filepath.Join(outDir, ".agents", "plugins", "wipnote", "agents", "old-agent")
	...
	// Stale command file must be gone (.agents/plugins/wipnote/ subtree was fully cleaned).
	...
	// Sibling plugin must STILL EXIST (cleanup was narrowed to wipnote/ only).
...
func TestCodexMarketplaceSourcePathForNestedSubdir(t *testing.T) {
	...
	codexTarget.PluginSubdir = ".agents/plugins/nested/wipnote"
	...
	pluginManifest := filepath.Join(outDir, ".agents", "plugins", "nested", "wipnote", ".codex-plugin", "plugin.json")
	if _, err := os.Stat(pluginManifest); err != nil {
